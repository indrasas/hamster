    ...
    if response.status_code == 200:
        ...
        try:
            ...
            if error_response.get('error_code') == 'INSUFFICIENT_FUNDS':
                print(Fore.RED + Style.BRIGHT + f"\r[ Upgrade Minning ] : Coin tidak cukup", flush=True)
...
if auto_upgrade_passive == 'y':
    while True:
        ...
        if max_price:
            ...
            print("Masukkan harga maksimum upgrade!.")
...
# Extract the morse code and daily cards from the data
cipher_text = data["morseCode"]
combo_list = data["dailyCards"]

if ask_cipher == 'y':
    print(f"The cipher text (daily code) is: {cipher_text}")

auto_claim_daily_combo = input("Auto Claim Daily Combo? (default n) (y/n): ").strip().lower() or 'n'

if auto_claim_daily_combo == 'y':
    for i, combo in enumerate(combo_list, start=1):
